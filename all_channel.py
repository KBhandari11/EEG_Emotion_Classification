...
def graph(epoch, loop):
     for x in range (0, loop):
        ...
        plt.show()
def epoch(data):
    raw = np.zeros((40,40,63,128)) # [video, channel, time(s), time points] (40, 40, 63, 128)
    ...
    return raw, norm  #(40, 40, 60, 128)

def feature_extraction(raw):
    psd = np.zeros((40, 40, 60, 64))
    f = np.zeros((40, 40, 60, 64))
    min_max_scaler = preprocessing.MinMaxScaler()
    for x in range(0,40):
        for y in range(0,19):
            for z in range(0,60):
...
def data_collection():
    #epoch_data = np.zeros((40, 40, 60, 64), dtype='f') #while using PS
    epoch_data = np.zeros((40, 40, 60, 128), dtype='f') #while using raw/norm
    epoch_data_i = np.zeros((40, 60, 128), dtype='f')
    ...
    for x in range (1,33):
            ...
            epoch_data_i, epoch_norm_i= epoch(data) # = (40, 40, 60, 128)  ; ie, [video, channel, time , timepoints)]
            #epoch_data_i = feature_extraction(epoch_data_i) # = (40, 40, 60, 64) 
    ...
    epoch_data= np.delete(epoch_data, slice(0,40), axis=0) #(1320, 40, 60, 128) =====> (1280, 19, 60, 128)
