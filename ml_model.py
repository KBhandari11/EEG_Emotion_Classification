...
class CNN_Model:
    ...
    def train(self):
        ...
        #first layer
        model.add(layers.Conv2D(40, self.kern, activation='relu',padding='SAME', input_shape=(self.size)))
        model.add(layers.MaxPooling2D((2, 2)))
        #second layer
        model.add(layers.Conv2D(20, self.kern,padding='SAME', activation='relu'))
        model.add(layers.MaxPooling2D((2, 2)))
        ...
        model.compile(loss='categorical_crossentropy',
              optimizer="sgd",
              metrics=['accuracy'])
        valence_history = model.fit(self.train_dataset,batch = 20, epochs=5)
